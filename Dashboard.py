...
classifier = load_model('training_model.h5')			#loading the model
...
def openimg():
    ...
    image = cv2.imread('template111.png')
    cv2.imshow("Image",image)
    cv2.setWindowProperty("Image",cv2.WND_PROP_FULLSCREEN,cv2.WINDOW_FULLSCREEN)
    cv2.resizeWindow("Image",720,720)
    cv2.moveWindow("Image", 1130,0)
...
def predictor():
    ...
    for i in range(len(fileEntry)):
        ...
        if(abs(len(good_points)+len(matches))>20):			#goodpoints and mataches sum from 1.png and customgestureimages is grater than 20
            gesname=fileEntry[i]
...
class Dashboard(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        cv2.destroyAllWindows()
        self.setWindowIcon(QtGui.QIcon('icons/Digital_Voice.png'))
        self.title = 'Digital Voice'
        ...
        movie = QtGui.QMovie("icons/sent_ges.gif")
        self.label_3.setMovie(movie)
        self.label_3.setGeometry(0,160,1111,711)
        movie.start()
    ...
    def createGest(self):
        ...
        while True:
            ...
            try:
                frame=cv2.resize(frame,(471,461))
                frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
                img2 = cv2.rectangle(frame, (150,50),(300,200), (255,255,255), thickness=2, lineType=8, shift=0)
    ...
    def audio(self):
        ...
        content=checkFile()
        self.textBrowser_4.setText("		 "+content)
    ...
    def exportFile(self):
        ...
        # self.pushButton_69.clicked.connect(self.exportFile)
        # self.pushButton_69.clicked.connect(self.on_click)
    ...
    def scanSent(self):
        ...
        while True:
            ...
            try:
                frame=cv2.resize(frame,(471, 461))
                frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
                img = cv2.rectangle(frame, (150,50),(300,200), (255,255,255), thickness=2, lineType=8, shift=0)
            except:
                keyboard.press_and_release('esc')
                keyboard.press_and_release('esc')
            ...
            cv2.setWindowProperty("mask",cv2.WND_PROP_FULLSCREEN,cv2.WINDOW_FULLSCREEN)
            cv2.resizeWindow("mask",219,145)
            cv2.moveWindow("mask", 860,300) # x,y mask pos wrt ui pos
            ...
            if keyboard.is_pressed('c'):
                try:
                    ...
                    if not os.path.exists('./TempGest'):    
                        os.mkdir('./TempGest')
                    img_name = ("./TempGest/"+"{}{}.png".format(str(counts),str(img_text)))
                    save_imgs = cv2.resize(mask1, (image_x, image_y))
                    cv2.imwrite(img_name, save_imgs)
        ...
        try:
            ...
            pass
        """Audio File Reading"""
        if self.pushButton_69.clicked.connect(self.audio):
           pass 
    ...
    def scanSingle(self):
        ...
        while True:
            ...
            try:
                frame=cv2.resize(frame,(471,461))
                frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
                img1 = cv2.rectangle(frame, (150,50),(300,200), (255,255,255), thickness=2, lineType=8, shift=0)
            except:
                keyboard.press_and_release('esc')
            ...
            cv2.imshow("mask", mask)
            cv2.setWindowProperty("mask",cv2.WND_PROP_FULLSCREEN,cv2.WINDOW_FULLSCREEN)
            cv2.resizeWindow("mask",135,127)
            cv2.moveWindow("mask", 960,300)
